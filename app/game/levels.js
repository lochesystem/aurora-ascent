...
export function generateLevel(levelNumber){
  ...
  const enemySpawns=platforms.map((platform,index)=>({platform,index,patrol:Math.min(1.05,Math.max(.55,Math.min(platform.s[0],platform.s[2])/2-1.25))})).filter(({platform,index})=>index<platforms.length-1&&!isTilted(platform)&&!["beam","slab","scaffold","column","small"].includes(platform.kind)&&Math.min(platform.s[0],platform.s[2])>=3.55).map(({platform,index,patrol})=>({platform,patrol,spawn:findEnemySpawn(platform,index,platforms,patrol,index+number)})).filter(candidate=>candidate.spawn);
